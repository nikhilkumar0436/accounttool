    ...
    <div class="invoice-container">
      ...
      <div class="table-container">
        ...
        <div *ngIf="invoices.length === 0" class="no-data">
          ...
          <p>No sales invoices found. Create your first sales invoice to get started!</p>
        </div>
      </div>
    </div>
  `,
...
export class SalesInvoiceListComponent implements OnInit {
  ...
  loadInvoices(): void {
    this.invoiceService.getGSTInvoices().subscribe({
      next: (invoices) => {
        // Filter for sales invoices
        this.invoices = invoices.filter(inv => inv.invoiceType === 'SALES');
      },
      error: (error) => {
        console.error('Error loading invoices:', error);
        this.snackBar.open('Failed to load sales invoices', 'Close', { duration: 3000 });
      }
    });
  }
  ...
  openInvoiceDialog(invoice?: GSTInvoice): void {
    const dialogRef = this.dialog.open(InvoiceFormComponent, {
      width: '1000px',
      maxHeight: '90vh',
      data: { invoice, invoiceType: 'SALES' }
    });
